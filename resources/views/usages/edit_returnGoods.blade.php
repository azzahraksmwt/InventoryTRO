...
@section('container')
    <div class="mx-4 my-8">
        ...
        <form action="{{ route('usages.update', $usage->idpb) }}" method="POST" class="no-select" id="usageForm"
            ...
            <input type="hidden" name="idPengguna" id="idPengguna" class="border border-gray-300 rounded w-full px-3 py-2"
                value="{{ $usage->idPengguna }}" readonly>
            <input type="hidden" name="status_pengembalian" id="status_pengembalian"
                class="border border-gray-300 rounded w-full px-3 py-2" value="{{ $usage->status_pengembalian }}" readonly>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="quantity_pinjam" id="quantity_pinjam"
                    class="border border-gray-300 rounded w-full px-3 py-2" value="{{ $usage->quantity_pinjam }}" readonly>
            </div>

            <div class="mb-4">
                <label for="tanggal_pinjam" class="block text-gray-700">Borrowed Date</label>
                <input type="date" name="tanggal_pinjam" id="tanggal_pinjam"
                    class="border border-gray-300 rounded w-full px-3 py-2" value="{{ $usage->tanggal_pinjam }}" readonly>
            </div>

            <div class="mb-4">
                <label for="idMatakuliah" class="block text-gray-700">Subject</label>
                <input type="text" name="idMatakuliah" id="idMatakuliah"
                    class="border border-gray-300 rounded w-full px-3 py-2" value="{{ $usage->subject->namaMatakuliah }}"
                    readonly>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="brg_rusak" id="brg_rusak"
                    class="border border-gray-300 rounded w-full px-3 py-2" min="0"
                    max="{{ $usage->quantity_pinjam }}" value="0">
            ...
            <a href="{{ route('usages.index') }}" class="bg-slate-700 text-white px-4 py-2 rounded-md">Cancel</a>
        ...
        function terimaPemakaian() {
            ...
            if (brgRusak > quantityPinjam) {
                document.getElementById('quantityError').innerText =
                    "damaged or lost items cannot exceed the borrowed items";
            } else {
                Swal.fire({
